...
fil <- function(array, top = 1, bottom = 0){
  array[array<top & array > bottom]
}
source("sample-once-2d.r")
source("four-estimators.r")
  ...
  out <- tryCatch(
    {
      ...
      sigma.array <- lapply(1:size, function(x) unlist(sigma.new(x)))
      # mean.array <- lapply(1:size, function(x) df[,1:(2 + noise.dim)][x, ])
      mean.array <- lapply(1:size, function(x)  colMeans(x.old[which(model$cluster == x),]))
      df$sigma <- sigma.array
