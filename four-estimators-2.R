  ...
  out <- tryCatch(
    {
      ...
      totss.tmp <- model$totss
      for (j in 1:10){
        model.tmp <- kmeans(x.old, size)
        if (model.tmp$totss < totss.tmp){
          model <- model.tmp
          totss.tmp <- model$totss
          print(j)
        }
      ...
      ## ------------------------------------------------------------------------
      sigma.array <- lapply(1:size, function(x) unlist(sigma.new(x)))
      mean.array <- lapply(1:size, function(x) colMeans(x.old[which(model$cluster == x),])) # lapply(1:size, function(x) df[,1:2][x, ])
      df$sigma <- sigma.array
      ...
      x.new <- ldply( 
        ...
        .fun = function(x) {
          mvrnorm(1,
                  mu = mean.array[[x]], #c(unlist(mean.array[[x]]),0),
                  Sigma = sigma.array[[x]])
        }
      )
...
results2000 <- sample.once()
for (tmp.counter in 1:999){
  results2000 <- rbind(results2000, sample.once())
}

mean((results2000[4,]-0.5)^2) 
mean((results2000[3,]-0.5)^2)
mean((results2000[2,]-0.5)^2)
mean((results2000[1,]-0.5)^2)
# 
# > mean((results2000[4,]-0.5)^2)
# [1] 0.0008764932
# > mean((results2000[3,]-0.5)^2)
...
source('~/Documents/GitHub/Importance-Sampling/four-estimators-2.RData')
